...
url = f"{SUPABASE_URL}/rest/v1/artifacts?pipeline_id=eq.{pipeline_id}"
response = requests.get(url, headers=headers)
artifacts = response.json()

for art in artifacts:
    print(f"--- Stage: {art['stage_name']} (Version: {art['version']}) ---")
    if art['stage_name'] == 'critic':
        print("Critic Feedback:")
        print(art['content'].get('feedback', 'No feedback field'))
